...
def forward_message(message):
  if message.text.startswith(settings['trigger_tag']) and message.chat.id != settings['outer_chat_id']:
    try:
      bot.forward_message(settings['outer_chat_id'], message.chat.id, message.message_id)
    except:
